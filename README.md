...
- фоновая проверка принятой доступности доменов и автоматический переподбор при ухудшении;
...
- не пытается определять пользователя или тип текущего соединения: безопасный запуск через проводной `eth0` остаётся ответственностью пользователя;
...
Рекомендуется запускать эту команду из SSH-сессии, подключённой через проводной `eth0`. Предварительный `sudo su -` не требуется, но не мешает установке. Скрипт намеренно не определяет адрес или маршрут пользователя: перед запуском самостоятельно убедитесь, что управление Raspberry Pi не зависит от `wlan0`.
