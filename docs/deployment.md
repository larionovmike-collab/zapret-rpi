...
Перед изменениями установка проверяет Debian/arm64, наличие `eth0` и `wlan0`, IPv4/default route на `eth0`, активный SSH, маршрут текущей SSH-сессии через `eth0` и отсутствие конфликта с `10.77.0.0/24`. Конфигурации проходят `dnsmasq --test` и `nft -c`; hostapd проверяется фактическим запуском project-owned unit с автоматическим откатом при ошибке. У hostapd ключ `-t` включает timestamps и не является режимом проверки конфигурации.
